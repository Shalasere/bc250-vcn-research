...
**Net effect:** the community's blind call had two independent, compounding problems — the wrong address (ABI violation) and, even if that were fixed, a currently-closed precondition gate. We did not attempt a live call to the corrected address: the gate-taken branch's target turned out to re-enter another `entry`-tagged code region rather than a simple early return, which needs more control-flow tracing before a live invocation would be informative rather than just a differently-caused hang. This is recorded here so nobody else spends time re-deriving the address correction or the gate location.

...
1. **PSP code execution** via `CVE-2023-31316` — public vulnerability in AMD Secure Processor's TMR handling during power save/restore. Community researcher (Discord handle) was actively working on this as of early September 2026 but reportedly blocked on an uninitialized `saved_len` field. If weaponized, would allow issuing `svc #0x7c` from inside PSP with authority to reach VCN. Would also allow reversing the SEC_GASKET writes at runtime.
